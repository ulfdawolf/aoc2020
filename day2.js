...
const parseInput = (string) => {
    const regex = new RegExp(/(\d{1,2})-(\d{1,2})\s(\w):\s(\w+)/g);

    const [_, min, max, letter, password] = regex.exec(string);
    return { min, max, letter, password, passwordLen: password.length, charAtMin: password[min-1], charAtMax: password[max-1] };
}

const parsedInput = input.split('\n').map(value => parseInput(value));

const isPasswordOK1 = input => {
    const count = (input.password.match(new RegExp(input.letter, "g")) || []).length;
    return (count >= input.min && count <= input.max) ? 1 : 0;
}

const isPasswordOK2 = input => {
    if (input.charAtMin === input.letter && input.charAtMax !== input.letter) {
        return 1
    }
    if (input.charAtMin !== input.letter && input.charAtMax === input.letter) {
        return 1
...
const findOKPasswords1 = () => parsedInput.map(isPasswordOK1).reduce((prev, current) => prev + current, 0);
const findOKPasswords2 = () => parsedInput.map(isPasswordOK2).reduce((prev, current) => prev + current, 0);

console.log("Result AoC 2-1:", findOKPasswords1());
console.log("Result AoC 2-2:", findOKPasswords2());
