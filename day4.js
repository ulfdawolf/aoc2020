...
const mustKeys1 = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];
...
const oneof = (value, values) => values.some(hc => hc === value);

const match = (value, rgx) => {
    const a = value.match(rgx);
    return a
}

const checkHeight = (value, {cm, inch}) => {
    const regex = new RegExp(/(\d+)(\w+)/g);
    const [_, height, unit] = regex.exec(value);
    if (unit === 'cm') {
        return within(height, cm);
    } else if (unit === 'in') {
        return within(height, inch)
    }
    return false;
} 
...
const mustKeys2 =  [
    { check: 'byr', test: within, input: [1920, 2002] }, 
    { check: 'iyr', test: within, input: [2010, 2020] },
    { check: 'eyr', test: within, input: [2020, 2030] },
    { check: 'hgt', test: checkHeight, input: { cm: [150, 193], inch: [59, 76] } },
    { check: 'hcl', test: match, input: new RegExp(/^\#[0-9|a-f]{6}$/g) },
    { check: 'ecl', test: oneof, input: ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"] }, 
    { check: 'pid', test: match, input: new RegExp(/^\d{9}$/g) }
];

const checkPassportsDetails = (passport) => {
    const allOk = mustKeys2.every(must => {
        return must.test(passport[must.check], must.input);
    })
    return allOk
}
