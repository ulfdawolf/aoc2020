...
const countAnswers = (input) => {
    let answers = 0;
    input.forEach(group => {
        answers += new Set(group.join('')).size;
    })
    return answers;
}

const intersections = (sets) => {
    if (sets.length === 0) {
        throw new Error("Empty stuff");
    }
    if (sets.length === 1) {
        return sets[0]
    }
    const other = sets.slice(1);
    return new Set([...sets[0]].filter(answer => other.every(set => set.has(answer))));
}

countIntersectingAnswers = (input) => {
    let answers = 0;
    for (group of input) {
        answers += intersections(group.map(answers => new Set(answers))).size;
    }
    return answers;
}

console.log("Result AoC 6-1:", countAnswers(input));

console.log("Result AoC 6-2:", countIntersectingAnswers(input));