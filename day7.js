...
const input = `dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.
...
wavy black bags contain 4 dotted indigo bags, 1 light tan bag, 5 bright cyan bags.`.split('\n');

const parseContains = (string, wantedBag) => {
    if (string.includes('no other bags')) {
        return [[], false, 0];
    }
    let containsWantedBag = false;
    let totalBags = 0;
    let bags = string.split(',').map(bag => {
        const [_, count, bagColor] = new RegExp(/(\d+) (\w+ \w+) bag/g).exec(bag);
        if (!containsWantedBag && bagColor === wantedBag) {
            containsWantedBag = true;
        }
        totalBags += parseInt(count);
        return [parseInt(count), bagColor];
    });
    return [bags, containsWantedBag, totalBags];
}

const parseInto = (wantedBag, allRules, holdsWantedBag, holdsNotWantedBag) => (string) => {
    const [container, contains] = string.split(' bags contain ');
    if (!!allRules[container]) {
        throw new Error('Multiple rules for', container);
    }
    const [bags, holdsBag, totalBags] = parseContains(contains, wantedBag);

    allRules[container] = {bags, totalBags};
    if (holdsBag) {
        holdsWantedBag[container] = 1;
    } else {
        holdsNotWantedBag[container] = true;
    }
}

const checkParentBags = (allRules, holdsWantedBag, holdsNotWantedBag) => {
    let newBagsFound;
    let order = 1;  
    do {
        order++;
        newBagsFound = 0;
        Object.keys(holdsNotWantedBag).forEach(bag => {
            if (allRules[bag].bags.some(([_, internalBag]) => !!holdsWantedBag[internalBag])) {
                newBagsFound++;
                holdsWantedBag[bag] = order;
                delete holdsNotWantedBag[bag];
            }
        });
    } while (newBagsFound);
} 

const containedBags = (allRules, bagColor) => {
    const {bags, totalBags} = allRules[bagColor];

    if (bags.length === 0) {
        return 0;
    }
    return totalBags + bags.map(([count, bag]) => count * containedBags(allRules, bag)).reduce((prev,curr) => prev + curr, 0);
}

const findBagsThatHold = (bagColor) => {
    const allRules = {};
    const holdsWantedBag = {};
    const holdsNotWantedBag = {};
    input.map(parseInto(bagColor, allRules, holdsWantedBag, holdsNotWantedBag));
    checkParentBags(allRules, holdsWantedBag, holdsNotWantedBag);

    console.log("Result AoC 7-1:", Object.keys(holdsWantedBag).length);
    console.log("Result AoC 7-2:", containedBags(allRules, bagColor));
}

findBagsThatHold('shiny gold');